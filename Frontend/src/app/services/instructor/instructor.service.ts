...
export class InstructorService {
  ...
  getInstructorCourses(instructorId: number) {
    return this.http
      .get<any>(`${this.baseUrl}/Instructor/${instructorId}`)
      .pipe(
          ...
          const courseRequests = courseIds.map((courseId) =>
            this.http.get<any>(`${this.baseUrl}/Course/${courseId}`).pipe(
              switchMap((courseData) => {
                const assignmentsIds: number[] =
                  courseData.assignmentIds?.$values || [];
                const quizIds: number[] = courseData.quizIds?.$values || [];
                const lectureIds: number[] =
                  courseData.lectureIDS?.$values || [];
                ...
                return forkJoin({
                  assignments: forkJoin(
                    assignmentRequests.length
                      ? assignmentRequests
                      : [Promise.resolve([])]
                  ),
                  quizzes: forkJoin(
                    quizRequests.length ? quizRequests : [Promise.resolve([])]
                  ),
                  lectures: forkJoin(
                  ...
                  map(({ assignments, quizzes, lectures }) => {
                    const course = new Courses(
                      ...
                      quizzes.map(
                        (q) =>
                          new Quiz(q.id, q.title, q.description, q.courseId)
                      ),
                      lectures.map(
                        (l) =>
                          new Lecture(l.id, l.title, l.description, l.courseId)
                      )
                    );
                    return course;
                  })
  ...
  addAssignment(
    ...
    return this.http.get<any>(`${this.baseUrl}/Course/${courseId}`).pipe(
      switchMap((course) => {
        ...
        return this.http
          .post<Assignment>(`${this.baseUrl}/assignments`, assignmentPayload)
          .pipe(
            switchMap((createdAssignment) => {
              ...
              const submissionRequests = studentIds.map((studentId: number) =>
                this.http.post(
                  `${this.baseUrl}/submissions/${studentId}/${assignmentId}`,
                  {}
                )
              );
      ...
      switchMap((course) => {
        ...
        return this.http.post<Quiz>(`${this.baseUrl}/Quiz`, quizPayload).pipe(
          switchMap((createdQuiz) => {
            ...
            const submissionRequests = studentIds.map((studentId: number) =>
              this.http.post(
                `${this.baseUrl}/StudentQuiz/${studentId}/${quizId}`,
                {}
              )
            );

            return forkJoin(submissionRequests).pipe(map(() => createdQuiz));
          })
        );
      })
    );
  ...
  getCourseStudents(studentIds: number[]): Observable<Student[]> {
    const requests = studentIds.map((id) =>
      this.http
        .get<any>(`${this.baseUrl}/students/${id}`)
        .pipe(
          map(
            (res) =>
              new Student(
